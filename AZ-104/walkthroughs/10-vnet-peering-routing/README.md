...
Stop here and compare what you just did to the portal's own peering wizard: if you clicked **+ Add peering** from either VNet's Peerings blade, the portal would prompt you once and create *both* of these objects for you behind the scenes. The Bicep template in this lab created them as two separate, explicit resources instead — same end result, but now you've seen both halves individually rather than through a wizard that hides the seam.
